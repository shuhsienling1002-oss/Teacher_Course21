...
st.markdown("""
    <style>
    @import url('https://fonts.googleapis.com/css2?family=Noto+Sans+TC:wght@400;500;700&family=Patrick+Hand&display=swap');

    /* 全局背景：米色紙張質感 */
    .stApp { 
        background-color: #FDFBF7;
        background-image: radial-gradient(#E0E0E0 1px, transparent 1px);
        background-size: 20px 20px; /* 點點筆記本效果 */
    ...
    .header-container {
        ...
        box-shadow: 5px 5px 0px #FFCCBC; /* 珊瑚色陰影 */
        border-radius: 15px;
        padding: 25px;
        text-align: center;
        margin-bottom: 40px;
    ...
    .main-title {
        font-family: 'Patrick Hand', cursive; /* 手寫英文字體 */
        color: #FF7043; /* 暖橘紅 */
        font-size: 40px;
        font-weight: 700;
        margin: 0;
    ...
    .sub-title { color: #757575; font-size: 18px; margin-top: 5px; font-weight: 500; }
    ...
    .word-card {
        background: #FFFFFF;
        border-radius: 2px; /* 較方正 */
        padding: 15px 10px;
        text-align: center;
        border: 1px solid #EEEEEE;
        border-left: 5px solid #FFAB91; /* 左側色條 */
        height: 100%;
        margin-bottom: 15px;
    ...
    .word-card:hover { transform: translateY(-3px); box-shadow: 4px 4px 10px rgba(0,0,0,0.1); }
    
    .icon-box { font-size: 30px; margin-bottom: 5px; }
    .zh-word { font-size: 14px; color: #9E9E9E; font-weight: 500; }
    ...
    .sentence-box {
        ...
        margin-bottom: 15px;
        border-radius: 10px;
        border-bottom: 2px solid #E0E0E0; /* 底部線條 */
    }
    .sentence-amis { font-size: 18px; color: #FF7043; font-weight: 700; margin-bottom: 5px; }
    .sentence-zh { font-size: 15px; color: #616161; }
    ...
    .stButton>button:hover { background: #FF8A65; }
...
SENTENCES = [
    {"amis": "Kapahay a remiad.", 
     "zh": "好的天氣。", 
     "emoji": "🌤️", "file": "s_kapahay_a_remiad"},
     
    {"amis": "Katangasaan tu ku remiad.", 
     "zh": "到期了。", 
     "emoji": "🔚", "file": "s_katangasaan_tu_ku_remiad"},
     
    {"amis": "Kasuvucan nu maku anini a remiad.", 
     "zh": "今天是我的生日。", 
     "emoji": "🎂", "file": "s_kasuvucan_nu_maku"},
     
    {"amis": "Saremiad sa a maurad anini.", 
     "zh": "今天整天下著雨。", 
     "emoji": "🌧️", "file": "s_saremiad_sa_a_maurad"},
     
    {"amis": "Kai remiad a pataluma’en kami.", 
     "zh": "白天送我們回家。", 
     "emoji": "🚗", "file": "s_kai_remiad"},
     
    {"amis": "Saremiaden a pawali ku vuduy.", 
     "zh": "衣服需整天曬著。", 
     "emoji": "👕", "file": "s_saremiaden_a_pawali"},
     
    {"amis": "Katangasaan tu ku kasuvucan nu misu a remiad.", 
     "zh": "你的生日到了。", 
     "emoji": "🎉", "file": "s_katangasaan_tu_ku_kasuvucan"},
     
    {"amis": "Aya! Katawalan nu maku.", 
     "zh": "哎呀! 我忘記了。", 
     "emoji": "🤦", "file": "s_aya_katawalan"},
     
    {"amis": "Uradan a remiad utiih a dademak.", 
     "zh": "下雨天工作不方便。", 
     "emoji": "🏗️", "file": "s_uradan_a_remiad"},
]
        ...
        for folder in folders:
            for ext in extensions:
                ...
                if os.path.exists(path):
                    ...
                    return 
        st.markdown(f"<span style='color:#E64A19; font-size:12px; background:#FFCCBC; padding:2px 5px; border-radius:4px;'>🔇 缺音檔: {filename_base}</span>", unsafe_allow_html=True)
    else:
        try:
...
def show_learning_mode():
    st.markdown("<h3 style='color:#FF7043; text-align:center; margin-bottom:20px;'>📝 單字筆記 (Vocabulary)</h3>", unsafe_allow_html=True)
    ...
    st.markdown("---")
    st.markdown("<h3 style='color:#FF7043; text-align:center; margin-bottom:20px;'>📖 例句練習 (Sentences)</h3>", unsafe_allow_html=True)
...
def show_quiz_mode():
    st.markdown("<h3 style='text-align: center; color: #E64A19;'>🎯 小測驗 (Quiz)</h3>", unsafe_allow_html=True)
    ...
    if st.session_state.current_q == 0:
        ...
        target = data['target']
        st.markdown(f"""
        <div class="word-card" style="border-left-color:#FF7043;">
            <h3>🎧 聽聽看，這是哪個字？</h3>
        </div>
        """, unsafe_allow_html=True)
        ...
        st.markdown(f"""
        <div class="word-card" style="border-left-color:#FFCA28;">
            <h3>🧩 句子填空</h3>
            <h2 style="color:#5D4037;">{data['q'].replace('______', '<span style="color:#FF7043; text-decoration:underline;">___</span>')}</h2>
        </div>
        """, unsafe_allow_html=True)
        ...
        target = data['target']
        st.markdown(f"""
        <div class="word-card" style="border-left-color:#66BB6A;">
            <h3>🗣️ 這是什麼意思？</h3>
            <h3 style="color:#2E7D32;">{target['amis']}</h3>
        </div>
        """, unsafe_allow_html=True)
        ...
        st.markdown(f"""
        <div class="word-card" style="border-left-color: #FF7043; background: #FFF3E0;">
            <h1 style='color: #E64A19;'>測驗完成！</h1>
