...
if 'q1_data' not in st.session_state:init_quiz()--- 3. 介面呈現 ---def show_learning_mode():st.markdown("// VOCABULARY MODULE", unsafe_allow_html=True)cols = st.columns(3)for idx, item in enumerate(VOCABULARY):with cols[idx % 3]:display_amis = item['amis']if "kasuvucan" in display_amis:display_amis = "kasuvucan(kasubucan)"if "vuduy" in display_amis:display_amis = "vuduy(buduy)"st.markdown(f"""{item['emoji']}{display_amis}{item['zh']}""", unsafe_allow_html=True)play_audio(item['amis'], filename_base=item['file'])st.write("")st.markdown("---")st.markdown("// SENTENCE MATRIX", unsafe_allow_html=True)for item in SENTENCES:st.markdown(f"""{item['emoji']} {item['amis']}{item['zh']}""", unsafe_allow_html=True)play_audio(item['amis'], filename_base=item['file'])def show_quiz_mode():st.markdown("// SYSTEM TERMINAL QUIZ", unsafe_allow_html=True)st.progress((st.session_state.current_q) / 3)st.write("")if st.session_state.current_q == 0:data = st.session_state.q1_datatarget = data['target']st.markdown(f""" [聽力解碼] 聽聽看，這是哪個字？""", unsafe_allow_html=True)play_audio(target['amis'], filename_base=target['file'])st.write("")cols = st.columns(3)for idx, opt in enumerate(data['options']):with cols[idx]:if st.button(f"{opt['zh']}", key=f"q1_{idx}"):if opt['amis'] == target['amis']:st.balloons()st.success("解碼成功！ (Correct)")time.sleep(1)st.session_state.score += 1st.session_state.current_q += 1st.rerun()else:st.error("特徵不符，再試一次")elif st.session_state.current_q == 1:data = st.session_state.q2_datast.markdown(f""" [矩陣填空] 補全文本鏈條{data['q'].replace('___', '')}""", unsafe_allow_html=True)cols = st.columns(3)for i, opt in enumerate(data['opts']):with cols[i]:if st.button(opt, key=f"q2_{i}"):if opt.lower() in data['ans'].lower() or data['ans'].lower() in opt.lower():st.balloons()st.success("校準正確！ (Great)")time.sleep(1)st.session_state.score += 1st.session_state.current_q += 1st.rerun()else:st.error("核心錯誤")elif st.session_state.current_q == 2:data = st.session_state.q3_datatarget = data['target']st.markdown(f""" [語義分析] 這是什麼意思？{target['amis']}""", unsafe_allow_html=True)for idx, opt in enumerate(data['options']):if st.button(opt, key=f"q3_{idx}"):if opt == target['zh']:st.balloons()st.success("同步完成！ (Perfect)")time.sleep(1)st.session_state.score += 1st.session_state.current_q += 1st.rerun()else:st.error("運算偏移，再想一下")else:st.markdown(f"""評估完成！系統同步率: {st.session_state.score} / 3""", unsafe_allow_html=True)if st.button("重新加載核心 (Restart)"):init_quiz()st.rerun()def show_debug_info():st.markdown("---")files_audio = []if os.path.exists("audio"):files_audio = [f for f in os.listdir('audio') if f.endswith('.m4a') or f.endswith('.mp3')]if not files_audio:st.caption(" ⚡ SYSTEM NOTICE：檢測到本地音頻模組未加載。建立 audio 資料夾並放入音檔，即可啟用真人音訊。")--- 主程式 ---def main():st.markdown("""Remiad // OS日子、天氣與白晝·數碼數據庫核心講師：胡美芳 | 數據提供：胡美芳""", unsafe_allow_html=True)tab1, tab2 = st.tabs([" 數據單元 (Learn)", " 終端測試 (Quiz)"])with tab1:show_learning_mode()with tab2:show_quiz_mode()show_debug_info()if name == "main":main()
