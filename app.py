...
QUIZ_DATA = [
    {id 1, audio_path assetsaudio01_listeninglistening_wordstengil-a1-01.mp3, question_text 聆聽音檔，選出關聯的詞彙：, options [riyar, 'alo, fanaw, sa'owac], correct_text riyar},
    {id 2, audio_path assetsaudio01_listeninglistening_wordstengil-a1-02.mp3, question_text 聆聽音檔，選出關聯的詞彙：, options [korkor, rohayan, romakat, rotarot], correct_text romakat},
    {id 3, audio_path assetsaudio01_listeninglistening_wordstengil-a1-03.mp3, question_text 聆聽音檔，選出關聯的詞彙：, options [hadhad, hakhak, hawan, hafay], correct_text hafay},
    {id 4, audio_path assetsaudio01_listeninglistening_wordstengil-a1-04.mp3, question_text 聆聽音檔，選出關聯的詞彙：, options [tefo', 'okoy, tafokod, tafolod], correct_text tafokod},
    {id 5, audio_path assetsaudio01_listeninglistening_wordstengil-a1-05.mp3, question_text 聆聽音檔，選出關聯的詞彙：, options [fakar, tayhi, pitaw, tarakar], correct_text pitaw},
    {id 6, audio_path assetsaudio01_listeninglistening_wordstengil-a1-06.mp3, question_text 聆聽音檔，選出關聯的詞彙：, options [sariri', riri', siri, riyar], correct_text siri},
    {id 7, audio_path assetsaudio01_listeninglistening_wordstengil-a1-07.mp3, question_text 聆聽音檔，選出關聯的詞彙：, options [koleto, lokot, kewaw, kakorot], correct_text koleto},
    {id 8, audio_path assetsaudio01_listeninglistening_wordstengil-a1-08.mp3, question_text 聆聽音檔，選出關聯的詞彙：, options [siwoy, kodasing, konga, damay], correct_text konga},
    {id 9, audio_path assetsaudio01_listeninglistening_wordstengil-a1-09.mp3, question_text 聆聽音檔，選出關聯的詞彙：, options [mali', tikami, tilifi, pawli], correct_text tilifi},
    {id 10, audio_path assetsaudio01_listeninglistening_wordstengil-a1-10.mp3, question_text 聆聽音檔，選出關聯的詞彙：, options [picakay, pitangtang, picaliw, pafeli'], correct_text picakay},
    {id 11, audio_path assetsaudio01_listeninglistening_wordstengil-a1-11.mp3, question_text 聆聽音檔，選出關聯的詞彙：, options ['olaw, 'alo, fao, tao], correct_text tao},
    {id 12, audio_path assetsaudio01_listeninglistening_wordstengil-a1-12.mp3, question_text 聆聽音檔，選出關聯的詞彙：, options [rorang, kolong, lotong, ekong], correct_text lotong},
    {id 13, audio_path assetsaudio01_listeninglistening_wordstengil-a1-13.mp3, question_text 聆聽音檔，選出關聯的詞彙：, options [Halitamako, Haliradiw, Haliepah, Hali'ecaw], correct_text Haliepah},
    {id 14, audio_path assetsaudio01_listeninglistening_wordstengil-a1-14.mp3, question_text 聆聽音檔，選出關聯的詞彙：, options [dafak, a'ayad, dadaya, kamaya], correct_text dadaya},
    {id 15, audio_path assetsaudio01_listeninglistening_wordstengil-a1-15.mp3, question_text 聆聽音檔，選出關聯的詞彙：, options [sioy, simal, sinafel, simico], correct_text sinafel}
]
...
def load_question_bank()
    ...
    for line in target_content.split(n)
        line = line.strip()
        # 遇到空行代表題目結束，存入題庫
        if not line
            save_question()
...
def render_mcq(line, prefix)
    ...
    try
        ...
        if is_listening
            if st.toggle(👁️ 顯示題目文字, key=ft_show_q_{prefix})
        ...
        if st.toggle(💡 顯示解答與分析, key=ft_ans_{prefix})
            if ans_str
                msg = f正確答案： {ans_str}
                if ana_str msg += fnn分析： {ana_str}
                st.success(msg)
            else
                st.warning(無標準答案。)
        elif user_ans and ans_str
            if ans_str in user_ans
                st.success(f✅ 正確！ + (f分析：{ana_str} if ana_str else ))
            else
                st.error(f❌ 錯誤。正確答案：{ans_str}。 + (f分析：{ana_str} if ana_str else ))
...
def render_reading(line, prefix)
    ...
    try
        ...
        st.markdown(f📖 {q_part})
        if ch_part
            if st.toggle(💡 顯示中文翻譯, key=ft_{prefix})
                st.success(ch_part)
    except
        st.info(line)
...
def render_qa(line, prefix)
    ...
    try
        ...
        if is_situational
            if st.toggle(👁️ 顯示題目與提示, key=ft_show_q_{prefix})
                st.markdown(f🗣️ {q_am})
                if ch_hint
                    st.caption(f中文提示：{ch_hint})
        else
            st.markdown(f🗣️ {q_am})
            if ch_hint
                st.caption(f中文提示：{ch_hint})
        ...
        if ans or ana
            if st.toggle(💡 顯示參考解答, key=ft_{prefix})
                msg = 
                if ans msg += f參考解答：{ans}
                if ana msg += fnn分析：{ana}
                st.success(msg)
    except
        st.info(line)
...
def render_picture(line, prefix)
    ...
    try
        ...
        try
            ...
            img_path_jpg = fassetsimagespicture_{idx}.jpg
            img_path_png = fassetsimagespicture_{idx}.png
            ...
            if os.path.exists(img_path_jpg)
                ...
                st.info(f🖼️ 圖片佔位區：若要顯示圖片，請將圖片命名為 `picture_{idx}.jpg` 或 `.png`，並放置於 `assetsimages` 資料夾中。)
        except
            pass # 若解析題號失敗則安全跳過
        ...
        if hint
            st.caption(f中文提示：{hint})
            
        # 加入輸入框作為草稿區
        st.text_area(請在此作答：, key=finput_{prefix}, label_visibility=collapsed, placeholder=可以在此輸入您的口說草稿...)
            
        if ans or ana
            if st.toggle(💡 顯示作答參考, key=ft_{prefix})
                msg = 
                if ans msg += f作答參考：{ans}
                if ana msg += fnn重點：{ana}
                st.success(msg)
    except
        st.info(line)
...
def render_dictation(line, prefix)
    ...
    try
        ...
        # 加入作答的文字輸入框，模擬真實寫作情境
        st.text_area(請在此作答：, key=finput_{prefix}, label_visibility=collapsed, placeholder=請在此輸入您聽寫的句子...)
        
        # 🌟 寫作測驗專屬：隱藏聽寫原文功能
        if st.toggle(👁️ 顯示聽寫原文, key=ft_show_dict_{prefix})
            st.markdown(f✍️ {am})
            
        if ch or ana
            if st.toggle(💡 顯示翻譯與分析, key=ft_{prefix})
                msg = 
                if ch msg += f中文：{ch}
                if ana msg += fnn分析：{ana}
                st.success(msg)
...
def main()
    ...
    # 🌺 台東阿美族女性傳統服飾風格 (Taitung Amis Female Attire Theme) CSS 視覺注入
    st.markdown(
    style
     全局背景 (柔和珍珠白米白，襯托鮮豔服飾色彩) 
    .stApp {
        background-color #FFFDF9;
        color #2D2522;
    }
    
     主標題樣式 (台東阿美女服經典亮桃紅朱紅色) 
    h1 {
    ...
    h2, h3 {
     ...
     題目卡片設計 (仿白裙基底 + 桃紅短上衣主色 + 翠綠滾邊刺繡感) 
    .quiz-card {
        background #FFFFFF;
        padding 24px;
        border-radius 12px;
        border 2px solid #E6005C;
        border-left 8px solid #00A86B;  象徵佩袋與圍裙的翠綠滾邊 
    ...
    hr {
     ...
     Segmented Control 導覽按鈕 (花帽與刺繡色彩風格) 
    ...
    if current_tab == 📋 認證考試說明
        st.subheader(📋 [認證考試說明](httpslokahsu.ilrdf.org.twweb_lokahsuFilesGuide1_20251211_162558.pdf))
        st.divider()
